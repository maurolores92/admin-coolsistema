...
import BizzeboIcon from 'src/@core/components/icon/BizzeboIcon'
    ...
    <ThemeProvider theme={darkTheme}>
      <Drawer {...props} navHover={navHover} setNavHover={setNavHover} navigationBorderWidth={navigationBorderWidth}>
        ...
        <Box sx={{ position: 'relative', overflow: 'hidden' }}>
          ...
          >
            ...
            {userNavMenuContent ? (
              ...
              <List className='nav-items' sx={{ pt: 0, mt: 4, '& > :first-child': { mt: '0' } }}>
                <VerticalNavItems
                  navHover={navHover}
            ...
            {/* {afterNavMenuContent && afterVerticalNavMenuContentPosition === 'static'
              ...
              : null} */}
            {/* Aqui puedo poner el boton vender negocio */}
            {user?.role.slug !== 'super_admin' ? (
              <Box sx={{ px: '10px' }}>
                <Button fullWidth style={{ backgroundColor: '#45b174', color: '#fff' }} onClick={toCreateBusiness}>
                  <BizzeboIcon icon='business' color={'#fff'} />{' '}
                  <span style={{ marginLeft: '10px' }}>Adquirir Mentoria</span>
                </Button>
              </Box>
            ) : (
              <></>
        ...
        {afterNavMenuContent && afterVerticalNavMenuContentPosition === 'fixed'
          ? afterNavMenuContent(navMenuContentProps)
          : null}
        {/* Aqui puedo poner el de usuario */}
        <UserMenu />
      </Drawer>
    </ThemeProvider>
  )
